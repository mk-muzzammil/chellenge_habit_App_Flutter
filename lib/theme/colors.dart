// import 'package:flutter/material.dart';

// class AppColors {
//   // Primary Colors
//   static const Color primary = Color(0xFF6C5DD3); // Example: Purple
//   static const Color primaryDark = Color(0xFF4A3CB2);
//   static const Color primaryLight = Color(0xFFA394F4);
//   static const Color skyBlue = Color(0xFF5DD8D0);
//   static const Color lightPink = Color(0xFFECB0CD);

//   // // Secondary Colors
//   // static const Color secondary = Color(0xFFF4A261); // Example: Orange
//   // static const Color secondaryDark = Color(0xFFB3743F);
//   // static const Color secondaryLight = Color(0xFFF7C99D);

//   // Neutral/Grayscale
//   static const Color background = Color(0xFF12151B); // Dark background
//   static const Color surface = Color(0xFF1E1E1E); // Cards/Containers
//   static const Color textPrimary = Color(0xFFFFFFFF); // White text
//   static const Color textSecondary = Color(0x80FFFFFF); // Gray text
//   static const Color textTertiary = Color(0xFFFFA2C0); // Gray text
//   static const Color divider = Color(0xFF2C2C2C);

//   // Alert Colors
//   static const Color success = Color(0xFF4CAF50); // Green
//   static const Color warning = Color(0xFFFFC107); // Yellow
//   static const Color error = Color(0xFFF44336); // Red
// }
import 'package:flutter/material.dart';

class AppColors {
  // ─────────────────────────────────────────────────────────────────────────────
  // LIGHT THEME COLORS
  // ─────────────────────────────────────────────────────────────────────────────
  static const Color lightPrimary = Color(0xFF6C5DD3); // Purple
  static const Color lightBackground = Colors.white;
  static const Color lightTextPrimary = Colors.black;
  static const Color lightTextSecondary = Colors.grey;
  // Add more if needed, e.g. surface, divider, etc.

  // ─────────────────────────────────────────────────────────────────────────────
  // DARK THEME COLORS
  // ─────────────────────────────────────────────────────────────────────────────
  static const Color darkPrimary = Color(0xFF6C5DD3); // Purple
  static const Color darkBackground = Color(0xFF12151B);
  static const Color darkTextPrimary = Color(0xFFFFFFFF);
  static const Color darkTextSecondary = Color(0x80FFFFFF);
  // Add more if needed, e.g. surface, divider, etc.
}
